...
class BallDetector:
    def __init__(self):
        ...
        self.ball_cov_pub = rospy.Publisher('/ball_cov', PoseWithCovarianceStamped, queue_size=5)
        ...
        self.moving_window = np.zeros(shape=(6, 100))
    ...
    def record_window(self, x, y, z):
        self.moving_window[0, self.moving_window_index] = x
        self.moving_window[1, self.moving_window_index] = y
        self.moving_window[2, self.moving_window_index] = z
        print(self.moving_window_index)
        
        if self.moving_window_index == 99:
            cov = np.cov(self.moving_window)
            print(cov.shape)
            print(cov)
            ...
            pwcs.pose.covariance = cov.flatten().tolist()
            
            
            self.ball_cov_pub.publish(pwcs)
            
            
        
        self.moving_window_index += 1
        self.moving_window_index %= 100
